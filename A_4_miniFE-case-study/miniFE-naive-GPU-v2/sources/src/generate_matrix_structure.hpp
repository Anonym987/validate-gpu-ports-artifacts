...
namespace miniFE
{
  ...
  int generate_matrix_structure(const simple_mesh_description<typename MatrixType::GlobalOrdinalType> &mesh,
                                MatrixType &A)
  {
    int myproc = 0;
#ifdef HAVE_MPI
    ...
    catch (std::exception &exc)
    {
      ...
      std::vector<GlobalOrdinal, UvmAllocator<GlobalOrdinal>> rows(nrows);
      std::vector<LocalOrdinal, UvmAllocator<LocalOrdinal>> row_offsets(nrows + 1);
      std::vector<LocalOrdinal, UvmAllocator<LocalOrdinal>> row_coords(nrows * 3);
      ...
      generate_matrix_structure_gpu(mesh,
                                    box, row_ptr,
                                    row_offset_ptr,
                                    row_coords_ptr,
                                    global_nodes_x, global_nodes_y,
                                    global_nodes_z, global_nrows);
