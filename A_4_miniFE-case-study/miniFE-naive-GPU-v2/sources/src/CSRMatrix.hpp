...
namespace miniFE
{
      ...
      CSRMatrix
  {
    ...
    std::vector<GlobalOrdinal, UvmAllocator<GlobalOrdinal>> rows;
    std::vector<LocalOrdinal, UvmAllocator<LocalOrdinal>> row_offsets;
    std::vector<LocalOrdinal, UvmAllocator<LocalOrdinal>> row_offsets_external;
    std::vector<GlobalOrdinal, UvmAllocator<GlobalOrdinal>> packed_cols;
    std::vector<Scalar, UvmAllocator<Scalar>> packed_coefs;
...
#pragma omp parallel for
                          ...
                          ScalarType *&coefs)
    {
      ...
      if (local_row == -1)
      {
        typename std::vector<GlobalOrdinal, UvmAllocator<GlobalOrdinal>>::iterator row_iter =
