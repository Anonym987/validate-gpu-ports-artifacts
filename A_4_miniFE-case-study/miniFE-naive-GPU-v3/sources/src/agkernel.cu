...
__device__ __managed__ double constant_scalars[2] = {0.0, 1.0};

void *cusparseBuffer = nullptr;
size_t cusparseBufferSize = 0;
                                     ...
                                     CVectorType &y)
{
    ...
    cusparseSpMatDescr_t Amat;

    CHECK_CUSP_ERROR(cusparseCreateCsr(&Amat, A.rows.size(), A.num_cols, A.num_nonzeros(),
                                       &A.row_offsets[0], &A.packed_cols[0], &A.packed_coefs[0],
                                       CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO,
                                       CUDA_R_64F));

    cusparseDnVecDescr_t xcoefs;
    cusparseDnVecDescr_t ycoefs;
    CHECK_CUSP_ERROR(cusparseCreateDnVec(&xcoefs, x.local_size, &x.coefs[0], CUDA_R_64F));
    CHECK_CUSP_ERROR(cusparseCreateDnVec(&ycoefs, y.local_size, &y.coefs[0], CUDA_R_64F));

    size_t bufferSize;
    CHECK_CUSP_ERROR(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                             &constant_scalars[1], Amat, xcoefs, &constant_scalars[0], ycoefs,
                                             CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
    if (cusparseBufferSize < bufferSize)
    {
        CHECK_CUDA_ERROR(cudaFree(cusparseBuffer));
        cusparseBuffer = nullptr;
        cusparseBufferSize = 0;
        CHECK_CUDA_ERROR(cudaMallocManaged(&cusparseBuffer, bufferSize));
        cusparseBufferSize = bufferSize;
    }
    CHECK_CUSP_ERROR(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
                                  &constant_scalars[1], Amat, xcoefs, &constant_scalars[0], ycoefs,
                                  CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, cusparseBuffer));

    CHECK_CUSP_ERROR(cusparseDestroyDnVec(ycoefs));
    CHECK_CUSP_ERROR(cusparseDestroyDnVec(xcoefs));
    CHECK_CUSP_ERROR(cusparseDestroySpMat(Amat));
