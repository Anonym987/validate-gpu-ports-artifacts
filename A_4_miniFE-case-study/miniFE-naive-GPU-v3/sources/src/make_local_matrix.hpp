...
namespace miniFE
{
  ...
  make_local_matrix(MatrixType &A)
  {
    ...
    // AG Add prefetch
    memPrefetchVectorToCPU(A.rows);
    memPrefetchVectorToCPU(A.row_offsets);
    memPrefetchVectorToCPU(A.packed_cols);
    memPrefetchVectorToCPU(A.packed_coefs);
    ...
    for (size_t i = 0; i < local_nrow; ++i)
    {
      ...
      {
        if (Acols[j] < 0)
        { // Change index values of externals
