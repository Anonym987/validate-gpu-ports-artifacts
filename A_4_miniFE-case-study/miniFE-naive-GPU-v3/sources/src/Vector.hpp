...
namespace miniFE
{
  ...
  struct Vector
  {
          ...
          local_size(local_sz)
    {
      ...
      cudaError_t err = cudaMallocManaged((void **)&coefs, sizeof(MINIFE_SCALAR) * local_size);
      if (err != cudaSuccess)
      {
        std::cerr << "CUDA Runtime Error in Vector"
                  << std::endl;
...
#pragma omp parallel for
    ...
    Vector(const Vector &other) : Vector(other.startIndex, other.local_size)
    {
      std::cout << "Copy Vector" << std::endl;
      const MINIFE_LOCAL_ORDINAL n = (MINIFE_LOCAL_ORDINAL)other.local_size;
#pragma omp parallel for
      for (MINIFE_LOCAL_ORDINAL i = 0; i < n; ++i)
      {
