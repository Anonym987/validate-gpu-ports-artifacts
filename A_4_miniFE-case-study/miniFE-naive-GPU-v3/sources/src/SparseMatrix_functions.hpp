...
namespace miniFE
{
  ...
  void init_matrix(MatrixType &M,
                   const std::vector<typename MatrixType::GlobalOrdinalType, UvmAllocator<typename MatrixType::GlobalOrdinalType>> &rows,
                   const std::vector<typename MatrixType::LocalOrdinalType, UvmAllocator<typename MatrixType::LocalOrdinalType>> &row_offsets,
                   ...
                   const simple_mesh_description<typename MatrixType::GlobalOrdinalType> &mesh)
  {
    ...
    init_matrix_gpu(M, rows, row_offsets, row_coords,
                    global_nodes_x, global_nodes_y, global_nodes_z, global_nrows, mesh);

    return;

    memPrefetchVectorToCPU(rows);
    memPrefetchVectorToCPU(row_offsets);
    memPrefetchVectorToCPU(row_coords);
    memPrefetchVectorToCPU(M.rows);
    memPrefetchVectorToCPU(M.row_offsets);
    memPrefetchVectorToCPU(M.packed_cols);
    memPrefetchVectorToCPU(M.packed_coefs);
...
#pragma omp atomic
                   ...
                   const Scalar *input_coefs)
  {
    for (size_t i = 0; i < num_inputs; ++i)
    {
      GlobalOrdinal *loc = gpu_matrix::lower_bound(row_indices, row_indices + row_len,
                                                   input_indices[i]);
      if (loc - row_indices < row_len && *loc == input_indices[i])
      {
        // if(flag && *loc==6)
        // std::cout<<"  ("<<*loc<<":"<<row_coefs[loc-row_indices]<<" += "<<input_coefs[i]<<")"<<std::endl;
        atomicAdd(row_coefs + (loc - row_indices), input_coefs[i]);
      }
    }
  }
               ...
               MatrixType &mat)
  {
    ...
    if (row_len == 0)
      return;
#ifdef __CUDACC__
    sum_into_row_gpu(row_len, mat_row_cols, mat_row_coefs, num_indices, col_inds, coefs);
#else
                          ...
                          MatrixType &mat)
  {
    ...
    for (size_t i = 0; i < num; ++i)
    {
      ...
      if (mat_row_len == 0)
        continue;
#ifdef __CUDACC__
      sum_into_row_gpu(mat_row_len, mat_row_cols, mat_row_coefs,
...
#ifdef __CUDACC__
    sum_into_vector_gpu(elem_data.nodes_per_elem, elem_data.elem_node_ids,
                   ...
                   const std::set<typename MatrixType::GlobalOrdinalType> &bc_rows)
  {
    ...
    impose_dirichlet_cuda(prescribed_value, A, b, global_nx, global_ny, global_nz, bc_rows);
    return;

    // MatrixType A_gpu(A);
    // VectorType b_gpu(b);
    // printf("\n\n\n=======BEGIN GPU");
    // impose_dirichlet_cuda(prescribed_value, A_gpu, b_gpu, global_nx, global_ny, global_nz, bc_rows);
    // printf("END GPU============\n\n\n");
...
#pragma omp atomic
    ...
    // vector_compare(A.packed_coefs, A_gpu.packed_coefs, 0.000000000001, "A.packed_coefs");
    // vector_compare(A.row_offsets_external, A_gpu.row_offsets_external, 0, "A.row_offsets_external");
    // vector_compare(A.row_offsets, A_gpu.row_offsets, 0, "A.row_offsets");
    // vector_compare(A.packed_cols, A_gpu.packed_cols, 0, "A.packed_cols");
    // vector_compare(A.rows, A_gpu.rows, 0, "A.rows");

    // for (size_t i = 0; i < b.local_size; i++)
    // {
    //   if (abs(b_gpu.coefs[i] - b.coefs[i]) > 0.00000001)
    //   {
    //     std::cerr << "b different at " << i << " b: " << b.coefs[i] << " b_gpu: " << b_gpu.coefs[i] << std::endl;
    //     std::exit(6);
    //   }
    // }
  }
