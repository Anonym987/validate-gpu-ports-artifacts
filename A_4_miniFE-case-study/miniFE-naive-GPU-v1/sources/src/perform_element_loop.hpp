...
namespace miniFE
{
                       ...
                       Parameters & /*params*/)
  {
    ...
    std::vector<GlobalOrdinal, UvmAllocator<GlobalOrdinal>> elemIDs(num_elems);
    ...
    // std::vector<GlobalOrdinal, UvmAllocator<GlobalOrdinal>> elemIDs_gpu(elemIDs);
    // MatrixType A_gpu(A);
    // VectorType b_gpu(b);

    // printf("\n\n\n=======BEGIN GPU");
    perform_element_loop_cuda(mesh, elemIDs, A, b);
    return;
    // printf("\n\n\n=======END GPU");
...
#pragma omp parallel for shared(elemIDs)
    ...
    // vector_compare(A.packed_coefs, A_gpu.packed_coefs, 0.00000001, "A.packed_coefs");
