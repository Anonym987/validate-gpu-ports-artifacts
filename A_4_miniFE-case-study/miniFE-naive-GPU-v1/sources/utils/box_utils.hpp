...
namespace miniFE
{
...
#endif
      void
      get_int_coords(GlobalOrdinal ID, int nx, int ny, int nz,
                     int &x, int &y, int &z)
  {
    z = ID / (nx * ny);
    y = (ID % (nx * ny)) / nx;
...
#endif
      void
      get_coords(GlobalOrdinal ID, int nx, int ny, int nz,
  ...
  inline bool is_neighbor(const Box &box1, const Box &box2)
  {
